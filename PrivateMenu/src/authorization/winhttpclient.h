...
public:
	...
	inline bool SetAdditionalDataToSend(uint8_t *data, unsigned int dataSize);
						...
						if (::WinHttpReceiveResponse(hRequest, NULL))
						{
							...
							do
							{
								...
								if (::WinHttpQueryDataAvailable(hRequest, &dwSize))
								{
									...
									uint8_t *pResponse = new BYTE[dwSize + 1];
									if (pResponse != NULL)
									{
											...
											&dwRead))
										{
											...
											{
												uint8_t *pOldBuffer = m_pResponse;
...
bool WinHttpClient::SetAdditionalDataToSend(uint8_t *data, unsigned int dataSize)
{
...
const uint8_t *WinHttpClient::GetRawResponseContent(void)
{
	return m_pResponse;
}
