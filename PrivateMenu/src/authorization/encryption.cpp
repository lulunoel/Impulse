...
string Encryption::base64encode2(uint8_t* data, size_t length) {
	std::string ret;
	int i = 0;
	int j = 0;
	unsigned char char_array_3[3];
...
string Encryption::aesEncrypt(string encryptionKey, string str) {
	...
	CryptoPP::StringSource s(keyStr, true, new CryptoPP::StreamTransformationFilter(cbcEncryption, new CryptoPP::StringSink(result)));
	return result;
}
...
string Encryption::aesDecrypt(string encryptionKey, string str) {
	...
	CryptoPP::StringSource s(str, true, new CryptoPP::StreamTransformationFilter(d, new CryptoPP::StringSink(result)));
	return result;
}
