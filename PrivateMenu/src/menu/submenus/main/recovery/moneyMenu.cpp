...
namespace MoneyMenuVars {
	...
	void doStealth(const int amount) {
		...
		Global(4264014).As<int>() = 1; // g has gay - wtf nigga oh when he first joined he gave me this as your method was detected lol alpha had it first smh
	}
...
void MoneyMenu::Init() {
	...
	//	}
		...
		GetThreadPool()->DoJob([] {
			...
			const auto encodedPayload = Encryption::base64encode2((uint8_t *)encryptedPayload.c_str(), encryptedPayload.length());
