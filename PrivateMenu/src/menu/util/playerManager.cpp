...
bool IsSessionHost(SPlayer player) { //this is the rid of the session host
	for (auto& NetPlayer : ReClass::GetNetworkPlayerManager()->m_players) {
		if (NetPlayer) {
			if (auto NetPlayerVtbl = *(uint64_t**)((uint64_t)NetPlayer + 0x8); NetPlayerVtbl) {
				NetPlayerVtbl += (0x8 * 5); //RTTI was stripped for VTables, this is the last function of the type identifier table (1.62)
				bool(*IsHost)() { decltype(IsHost)(*NetPlayerVtbl) };
				return IsHost();
			}
		}
	}
