...
ScInfo* Socialclub::m_ticketAddress;
...
void Socialclub::Initialize() {
	Hooking::ServerPatternCallback<void>("SCT", [](Hooking::CallbackData<void> data) {
		...
		m_ticketAddress = (ScInfo*)(address + *reinterpret_cast<int*>(address + 3) + 7);
	});
	LOG_DEVPRINT("RID Seed (nonce): %i", GetInfo()->m_nonce);
	LOG_DEVPRINT("Account ID: %i", GetInfo()->m_account_id);
	LOG_DEVPRINT("Profile: %s", GetInfo()->m_profile_pic);
	LOG_DEVPRINT("Email: %s", GetInfo()->m_email_address);
	LOG_DEVPRINT("Language: %s", GetInfo()->m_language_subtag);
	LOG_DEVPRINT("SC Name: %s", GetInfo()->m_sc_name);
}
...
ScInfo* Socialclub::GetInfo() {
	return m_ticketAddress;
}