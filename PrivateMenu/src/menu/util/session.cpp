...
void Session::GetSessionInfoByRockstarId(uint32_t handle, std::function<void(uint32_t, bool, const char *, int)> callback) {
	...
	sendSessionInfoByGamerHandleTask(0, &data, 1, &result, 1, &success, &state);
}
#pragma optimize("", on)
...
void Session::Initialize() {
	Hooking::ServerPatternCallback<char>("SSIHT", [](Hooking::CallbackData<char> data) {
		sendSessionInfoByGamerHandleTask = reinterpret_cast<sendSessionInfoByGamerHandleTaskType>(GetAddressFromInstruction(data.m_address, 0, 1, 5));
	});

	Hooking::ServerPatternCallback<char>("RSIR", [](Hooking::CallbackData<char> data) {
		Hooking::HookFunction((void*)GetAddressFromInstruction(data.m_address, 0, 1, 5), &_readSessionInfoResponse, &__readSessionInfoResponse, "RSIR");
	});

	Hooking::ServerPatternCallback<char>("SESRS", [](Hooking::CallbackData<char> data) {
		Hooking::HookFunction((void*)GetAddressFromInstruction(data.m_address, 0, 1, 5), &_readSessionInfo, &__readSessionInfo, "SESRSH");
	});

	Hooking::ServerPatternCallback<char>("SESJS", [](Hooking::CallbackData<char> data) {
		joinSelectedPlayer = reinterpret_cast<joinSelectedPlayerType>(GetAddressFromInstruction(data.m_address, 0, 1, 5));
	});
