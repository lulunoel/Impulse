...
struct TamperTable {
	...
	TamperTable(String _name, void* _address, uint8_t* _originalBytes) {
		m_name = _name;
		m_address = _address;
...
public:
	...
	static uint8_t RegisterAntiTamper(String name, void* address, uint8_t expectedFirst);

	template <typename T>
	static uint8_t HookProcAddress(HMODULE hModule, String func, void* hookFunction, T** trampoline, uint8_t expectedFirst);
