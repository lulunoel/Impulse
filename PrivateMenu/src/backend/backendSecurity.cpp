...
void Security::InternetDetection() {
	...
	if (ERROR_INSUFFICIENT_BUFFER == GetIpForwardTable(NULL, &dwBufferSize, false)) {
		uint8_t* pByte = new BYTE[dwBufferSize];
		if (pByte) {
...
uint8_t Security::RegisterAntiTamper(String name, void* address, uint8_t expectedFirst) {
...
template<typename T>
uint8_t Security::HookProcAddress(HMODULE hModule, String func, void* hookFunction, T** trampoline, uint8_t expectedFirst) {
	HMODULE mod = hModule;
